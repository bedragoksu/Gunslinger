...
{
    public class CardsControllerAI : NetworkBehaviour
    {
        [SerializeField] private NetworkManager _networkManager;
        public GameManagerAI gameManager;
        public PlayerRolesControllerAI prc;
        ...
        private void CheckTheNextCard(GameObject player)
        {
            ...
            var pointer = player.GetComponent<CardManagerAI>().CardOrder[CardPointer];
        ...
        public void DrawCardsServer(GameObject player, int amount)
        {
            ...
            for(int i=0; i < amount; i++)
            {
                ...
                var child = GetChildOfDeck(pointer,_deck);
                child.SetActive(true);
                player.GetComponent<PlayerModelAI>().openHand.Add(child);
        ...
        public void DiscardCards(GameObject player, int i)
        {
            ...
            var PlayerModelAI = player.GetComponent<PlayerModelAI>();
            ...
            PlayerModelAI.openHand[i].SetActive(false);
            PlayerModelAI.openHand.RemoveAt(i);

            if (player == gameManager._thisPlayer)
            {
                foreach (var c in gameManager._thisPlayer.GetComponent<PlayerModelAI>().openHand)
                {
                    Debug.Log("foreach");
        ...
        public void MoveToStack(GameObject player, int i)
        {
            ...
            {
                if(info.gameObject.name == PlayerModelAI.PlayerName)
                {
        ...
        public bool DealCards()
        {
            ...
            var serverCardManagerAI = Players[0].GetComponent<CardManagerAI>();
            int counter = 0;
            // assign roles to players
            foreach (var player in Players)
            {
                var plmodel = player.GetComponent<PlayerModelAI>();
                AssignCards(player, serverCardManagerAI.CardOrder);
        ...
        {
            var card = player.GetComponent<CardManagerAI>();

            //card.CardObjects = cards;
            card.CardOrder = cardOrder;

            var pl = player.GetComponent<PlayerModelAI>();
        ...
        [ServerRpc(RequireOwnership =false)]
        public void UpdateHealthServer(PlayerModelAI player, int amount) // 0 ve max burada kontrol et
        {
            UpdateHealth(player, amount);
        }
        [ObserversRpc]
        public void UpdateHealth(PlayerModelAI player, int amount)
        {
            player.CurrentBulletPoint += amount;// can 0 olmasi durumu/ dead !! bedra

            int maxpoint = 0;
            if (player.PlayerRole == PlayerModelAI.TypeOfPlayer.Sheriff)
            {
                 maxpoint = 5;
            }
            else
